...
\item Sets of names or identification numbers like ``CS101'' are not atomic
...
\item For example, $R=\{\text{team\_id},\text{team\_name},\text{trophies},\text{player\_id},\text{player\_name},\text{player\_position}\}$ with the obvious functional dependencies is not in BCNF, since \text{team\_id} is not a superkey of $R$
\end{arrows}
\end{itemize}
